...
window.onload = async () => {
    ...
    if(token) {
        ...
        .then(sessionData => {
            ...
            location.href = 'http://127.0.0.1:5500/index.html' //reload dans la barre de navigation
        })
        .catch(err => {
            console.error(err);
...
async function redirectUserToSSO() {
    ...
    location.href = `https://www.themoviedb.org/authenticate/${tokenData.request_token}?redirect_to=http://127.0.0.1:5500/index.html`
}
